...
Quick has one execution pass and one local review; Scoped also defaults to one pass. A Track plan groups work by independently deliverable outcome rather than one task per file, test, or documentation update. Keep ownership unambiguous and retain approval, rollout, and rollback gates. Changing the grouping of an already approved plan must not silently change its acceptance or reset budgets.

The default checkpoints are 15 minutes for Quick and 45 minutes for Scoped. Track names a checkpoint for each phase. At a checkpoint summarize elapsed time, first implementation completion if reached, unresolved acceptance, and the smallest remaining path. Continue within authorization when progress is concrete. If progress needs a missing capability, user decision, or exhausted hard budget, return partial/blocked with useful evidence. Do not manufacture a blocker merely because a soft checkpoint elapsed.
...
Repeated failure with the same cause and unchanged evidence is not progress. Do not retry it through another child or Work Order. Corrective and review-fix limits still apply; this rule does not authorize extra attempts.

## Accept And Stop

Accept only when required criteria and checks pass and the applicable review has no Blocking finding. At phase/final review, reconcile the original acceptance with accumulated evidence; execute only missing or invalidated checks. A real regression may reopen affected work, with its original budget. A new independent concern is a follow-up, not an automatic repair.
